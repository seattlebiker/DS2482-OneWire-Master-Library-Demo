...
// The original definition in user cybergibbon's code was incorrect 
// and only left-shifted the 1 bit once and defined the Presence Pulse Detected,
// instead of defining the SD bit (Short Detected) 2X so a short was 
// never detected.
#define DS248X_ERROR_SHORT			(1<<2)		 
#define DS248X_ERROR_CONFIG			(1<<3)
...
public:
	...
	boolean OWSearch(uint8_t *address, SearchMode mode);
	void	OWResetSearch();
