 ...
 * suffers from the same issue as follows:
 * 
 ...
 * PDF since I think it's helpful, instead of having to repeatedly refer to to the APP note.  
 * This has helped me during the code process, anyway.  :-)
 * 
 * =======================================================================
 * 
 ...
 * bit7_bit6 bit5 bit4 bit3 bit2 bit1 bit0
 *=======================
 *  DIR__TSB  SBR  RST   LL__SD  PPD 
 * 
 ******************************************************************/ 
 ...
 * 
 * Applicable Bit Descriptions from the DS2482 Device Specifications PDF
 * 
 * 1-Wire Busy (1WB) 
 ...
 *	1-Wire Reset command provided that the short has been removed. If SD is 1, PPD will be 0. The DS2482 cannot 
...
uint8_t DS248X::OWReset()
{
	...
	{
		mError = DS248X_ERROR_SHORT;	// Short Detected
	} else {
		mError = 0;		// For some reason OWReset() doesn't reset SD to 0
	}
	#ifdef DEBUG0
...
/**
  ...
  */   
boolean DS248X::OWSearch(uint8_t *dev_addr, SearchMode mode)
{
...
{       
	...
	for (i = 0; i < 8; i++)
 	...
 	if (DS248X::OWSearch(found, NORMAL_MODE)) 
	{ 		
...
void printBinary(uint8_t byteValue) {
  for (int i = 7; i >= 0; i--) {
